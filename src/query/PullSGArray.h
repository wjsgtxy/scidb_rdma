 ...
 */
class PullSGArrayBlocking
    : public SynchableArray
    , public PullSGArray
{
public:
    /**
    ...
    typedef std::function< void (const AttributeID attId,
...
public:
    ...
    _ev(),
    _cond(false),
    _ec(std::bind(&Query::isValidQueryPtr, std::weak_ptr<Query>(query)))
    {}

    SyncCtx(const std::weak_ptr<Query>& query) :
    _ev(),
    _cond(false),
    _ec(std::bind(&Query::isValidQueryPtr, query))
    {}
