...
public:
    ...
    public:
        ...
        void append(Coordinates const& chunkPos)
        {
            ...
            if (it == _map.end()) {
                _map.insert(std::pair<Key, std::shared_ptr<VectorOfChunkPos> >(key, std::make_shared<VectorOfChunkPos>()));
                it = _map.find(key);
                assert(it!=_map.end());
            }
    ...
    struct MyVariablesInExecute
    {
        ...
        vector<AttributeDesc> _inputAttrIDs;   /// the attributes in the input array, to compute aggregates on
    ...
    std::shared_ptr<MemArray> buildLocalEdges(CommonVariablesInExecute const& commonVars, MyVariablesInExecute const& myVars)
    {
        ...
        for (AttributeID i=0; i < aggrsCount; ++i) {  // DJG
            attrsEdge.push_back(AttributeDesc(
                    commonVars._output._attrsWithoutET.findattr(i).getName(),
                    myVars._aggregates[i]->getStateType().typeId(),
                    commonVars._output._attrsWithoutET.findattr(i).getFlags(),
                    commonVars._output._attrsWithoutET.findattr(i).getDefaultCompressionMethod()
                    ));
        }

        std::shared_ptr<MemArray> localEdges = std::make_shared<MemArray>(
            // ArrayDesc consumes the new copy, source is discarded.
            ArrayDesc(commonVars._output._schema.getName(),
                      attrsEdge.addEmptyTagAttribute(),
        ...
        // Fill in data.
        //
        for (AttributeID outputAttr = 0; outputAttr < myVars._numAggrs; ++outputAttr) {  // DJG
            std::shared_ptr<ConstArrayIterator> inputArrayIter =
                commonVars._input._array->getConstIterator(myVars._inputAttrIDs[outputAttr]);  // DJG
            const auto& localEdgesAttrs = localEdges->getArrayDesc().getAttributes();
            auto outputAttrIter = localEdgesAttrs.find(outputAttr);
            SCIDB_ASSERT(outputAttrIter != localEdgesAttrs.end());
            std::shared_ptr<ArrayIterator> localEdgesArrayIter = localEdges->getIterator(*outputAttrIter);
    ...
    {
        std::shared_ptr<MapOfVectorsOfChunkPos> mapOfVectorsInRemoteEdges = std::make_shared<MapOfVectorsOfChunkPos>(myVars._aggrDim);
        const auto& localEdgesAttrs = myVars._localEdges->getArrayDesc().getAttributes();
        const auto& fda = localEdgesAttrs.firstDataAttribute();
        std::shared_ptr<ConstArrayIterator> localEdgesArrayIter = myVars._localEdges->getConstIterator(fda);
    ...
    {
        std::shared_ptr<MapOfVectorsOfChunkPos> mapOfVectorsInInputArray = std::make_shared<MapOfVectorsOfChunkPos>(myVars._aggrDim);
    ...
    void doCumulate(CommonVariablesInExecute const& commonVars, MyVariablesInExecute const& myVars)
    {
        ...
        for (AttributeID outputAttr = 0; outputAttr < myVars._numAggrs; outputAttr++) {
            // array iterators
            //
            auto inputAttr = myVars._inputAttrIDs[outputAttr];
            std::shared_ptr<ConstArrayIterator> inputArrayIter = commonVars._input._array->getConstIterator(inputAttr);
            const auto& allEdgesAttrs = myVars._allEdges->getArrayDesc().getAttributes();
            auto outputAttrIter = allEdgesAttrs.find(outputAttr);
            SCIDB_ASSERT(outputAttrIter != allEdgesAttrs.end());
            std::shared_ptr<ConstArrayIterator> remoteEdgesArrayIter = myVars._allEdges->getConstIterator(*outputAttrIter);
            std::shared_ptr<ArrayIterator> outputArrayIter = commonVars._output._array->getIterator(*outputAttrIter);
            ...
            std::shared_ptr<Query> query)
    {
        ...
        std::shared_ptr<Array> outputArray = std::make_shared<MemArray>(_schema, query);
        ...
        for (size_t i = 0; i<myVars._numAggrs; i++) {
            ...
            //
            if (myVars._inputAttrIDs[i] == AttributeDesc()) {
                myVars._inputAttrIDs[i] = commonVars._input._attrsWithET.firstDataAttribute();
            }
        }
        ...
        //
        myVars._cellPosToKey = std::make_shared<CoordinatesToKey>();
