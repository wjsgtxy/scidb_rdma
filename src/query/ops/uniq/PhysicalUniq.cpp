...
private:
    ...
    struct InputChunkInfo
    {
         ...
         */
        const void* unmarshall(const void* ptr)
        {
            auto sptr = reinterpret_cast<const size_t*>(ptr);
            numUniqueElements = *sptr;
            ++sptr;
            if(numUniqueElements>=1)
            {
                size_t startingValueSize = *sptr;
                auto bptr = reinterpret_cast<const uint8_t*>(sptr+1);
                startingValue.setData(bptr, startingValueSize);
                bptr += startingValueSize;
                sptr = reinterpret_cast<const size_t*>(bptr);
                size_t endingValueSize = *sptr;
                bptr = reinterpret_cast<const uint8_t*>(sptr+1);
    ...
    struct OutputChunkInfo
    {
        ...
        const void* unmarshall(const void* ptr)
        {
            auto cptr = reinterpret_cast<const Coordinate*>(ptr);
            startingPosition = *cptr;
            ++cptr;
            auto bptr = reinterpret_cast<const bool*>(cptr);
    ...
    public:
        ...
        virtual void unMarshall(std::shared_ptr<SharedBuffer>& buf)
        {
            ...
            for ( ; numEntries > 0; --numEntries)
            {
                ...
                Element elt;
                auto cvPtr = reinterpret_cast<const void*>(cptr);
                auto eltPtr = elt.unmarshall(cvPtr);
                sptr = reinterpret_cast<const size_t*>(eltPtr);
    ...
    public:
        OutputArraySequentialWriter(ArrayDesc const& schema, std::shared_ptr<Query>& query):
            ...
            _outputArrayIterator(_output->getIterator(schema.getAttributes().firstDataAttribute())) //the chunk iterator is NULL at the start
        {
...
private:
    ...
    void fillInputArrayInfo(std::shared_ptr<Array>& inputArray, InputArrayInfo& inputArrayInfo)
    {
        ...
        std::shared_ptr<ConstArrayIterator> inputArrayIter =
            inputArray->getConstIterator(inputArrayAttrs.firstDataAttribute());
                                       ...
                                       std::shared_ptr<Query>& query)
    {
        ...
        const auto& inputArrayAttrs = inputArray->getArrayDesc().getAttributes();
        for (std::shared_ptr<ConstArrayIterator> inputArrayIter =
             inputArray->getConstIterator(inputArrayAttrs.firstDataAttribute());
             !inputArrayIter->end();
...
public:
    ...
    std::shared_ptr<Array> execute(vector< std::shared_ptr< Array> >& inputArrays, std::shared_ptr<Query> query)
    {
        ...
        inputArrayInfo.dumpToLog(inputArrays[0]->getArrayDesc().getAttributes().firstDataAttribute().getType());
