  ...
  public:
    ...
    void checkInputAttributes(const Attributes& attrs,
      ...
      for (AttributeID i = 1; i < nAttrs; ++i) {
        if (attrs.findattr(i).getType() != attrs.findattr(0).getType()) {
    ...
    ArrayDesc inferSchema(vector<ArrayDesc> schemas,
      ...
      outputAttributes.push_back(AttributeDesc(attrs.findattr(0).getName(),
					       attrs.findattr(0).getType(),
      ...
      // Don't add an empty bitmap unless the input has one.
