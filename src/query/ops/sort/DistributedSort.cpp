...
#include <network/Network.h>   // For BufBroadcast
...
: _query(query),
  ...
  _tupleComparator(std::make_shared<TupleComparator>(sortingAttributeInfos, _schemaUtils._schema)),
  ...
  _timing(timing)
{
    ...
    const auto& sortedLocalDataAttrs = sortedLocalData->getArrayDesc().getAttributes();
    for (SortingAttributeInfos::const_iterator it = _sortingAttributeInfos.begin(); it != _sortingAttributeInfos.end(); ++it) {
        ...
        auto attrIter = sortedLocalDataAttrs.find(attrID);
        SCIDB_ASSERT(attrIter != sortedLocalDataAttrs.end());
        _sortedLocalDataArrayIterators[attrIter->getId()] = sortedLocalData->getConstIterator(*attrIter);
    }
...
void DistributedSort::fillSplitterFromChunkIterators(size_t localIndex, Splitter & splitter)
{
    ...
    const auto& sortedLocalDataAttrs = _sortedLocalData->getArrayDesc().getAttributes();
    auto oneAttrIter = sortedLocalDataAttrs.find(oneAttr);
    SCIDB_ASSERT(oneAttrIter != sortedLocalDataAttrs.end());
    std::shared_ptr<ConstChunkIterator>& oneChunkIter = _sortedLocalDataChunkIterators[oneAttrIter->getId()];
    std::shared_ptr<ConstArrayIterator>& oneArrayIter = _sortedLocalDataArrayIterators[oneAttrIter->getId()];
    ...
    if (!oneChunkIter || oneChunkIter->end() || localIndex != coords2LocalIndex(oneChunkIter->getPosition())) {
        ...
        if (!oneChunkIter || oneChunkIter->end() ||
            ...
            for (SortingAttributeInfos::const_iterator it = _sortingAttributeInfos.begin(); it != _sortingAttributeInfos.end(); ++it) {
                ...
                auto attrIter = sortedLocalDataAttrs.find(attrID);
                SCIDB_ASSERT(attrIter != sortedLocalDataAttrs.end());
                assert(_sortedLocalDataArrayIterators[attrIter->getId()]);
                const bool mustSucceed = _sortedLocalDataArrayIterators[attrIter->getId()]->setPosition(chunkPos);
                SCIDB_ASSERT(mustSucceed);
                _sortedLocalDataChunkIterators[attrIter->getId()] = _sortedLocalDataArrayIterators[attrIter->getId()]->getChunk().getConstIterator();
            }
        }
        ...
        for (SortingAttributeInfos::const_iterator it = _sortingAttributeInfos.begin(); it != _sortingAttributeInfos.end(); ++it) {
            ...
            auto attrIter = sortedLocalDataAttrs.find(attrID);
            SCIDB_ASSERT(attrIter != sortedLocalDataAttrs.end());
            assert(_sortedLocalDataChunkIterators[attrIter->getId()] && !_sortedLocalDataChunkIterators[attrIter->getId()]->end());
            const bool mustSucceed = _sortedLocalDataChunkIterators[attrIter->getId()]->setPosition(cellPos);
            SCIDB_ASSERT(mustSucceed);
        }
    }
...
void DistributedSort::buildFirstRecordInEachChunk()
{
    ...
    for (SortingAttributeInfos::const_iterator it = _sortingAttributeInfos.begin(); it != _sortingAttributeInfos.end(); ++it) {
        ...
        auto attrIter = sortedLocalDataAttrs.find(attrID);
        SCIDB_ASSERT(attrIter != sortedLocalDataAttrs.end());
        _sortedLocalDataArrayIterators[attrIter->getId()]->restart();
    }
    ...
    auto oneAttrIter = sortedLocalDataAttrs.find(oneAttr);
    SCIDB_ASSERT(oneAttrIter != sortedLocalDataAttrs.end());
    while (! _sortedLocalDataArrayIterators[oneAttrIter->getId()]->end()) {
        ...
        for (SortingAttributeInfos::const_iterator it = _sortingAttributeInfos.begin(); it != _sortingAttributeInfos.end(); ++it) {
            ...
            auto attrIter = sortedLocalDataAttrs.find(attrID);
            SCIDB_ASSERT(attrIter != sortedLocalDataAttrs.end());
            ++(*_sortedLocalDataArrayIterators[attrIter->getId()]);
        }
    }
    ...
    if (! _firstRecordInEachChunk.empty()) {
        ...
        for (SortingAttributeInfos::const_iterator it = _sortingAttributeInfos.begin(); it != _sortingAttributeInfos.end(); ++it) {
            ...
            auto attrIter = sortedLocalDataAttrs.find(attrID);
            SCIDB_ASSERT(attrIter != sortedLocalDataAttrs.end());
            _sortedLocalDataArrayIterators[attrIter->getId()]->setPosition(lastChunkPos);
        }
        size_t numRecordsInLastChunk = _sortedLocalDataArrayIterators[oneAttrIter->getId()]->getChunk().count();
...
void DistributedSort::determineGlobalMinMaxSplitterAndCounts(SplitterAndCounts& minSplitterAndCounts, SplitterAndCounts& maxSplitterAndCounts)
{
    ...
    for (InstanceID i=0; i<_numInstances; ++i) {
        ...
        boost::archive::binary_iarchive* iArchive = iArchiveWrapper.reset(buffer);
        size_t localCount = 0;
        (*iArchive) & localCount;
...
std::shared_ptr<MemArray> DistributedSort::distributeBasedOnAnchors()
{
    ...
    for (size_t i=0; i<_numInstances; ++i) {
        outboundArrays[i] = std::make_shared<MemArray>(_schemaUtils._schema, _query);
    }
    ...
    std::shared_ptr<RemoteArrayContext> remoteArrayContext = std::make_shared<RemoteArrayContext>(_numInstances);
    ...
    for (InstanceID i = 0; i < _numInstances; i++) {
        ...
        if (Config::getInstance()->getOption<int>(CONFIG_RESULT_PREFETCH_QUEUE_SIZE) > 1
            ...
            outboundArrays[i] = std::make_shared<AccumulatorArray>(outboundArrays[i], _query);
        }
    ...
    std::shared_ptr<vector<size_t>> streamSizes = std::make_shared<vector<size_t>>(_numInstances);
    for (size_t i=0; i<_numInstances; ++i) {
    ...
    std::shared_ptr<Array> mergeSortResult = std::make_shared<MergeSortArray>(
...
std::shared_ptr<MemArray> DistributedSort::sort()
{
    ...
    if (maxSplitterAndCounts._globalCount == 0) {
        return std::make_shared<MemArray>(_schemaUtils._schema, _query);
    }
    ...
    while (true) {
        ...
        for (InstanceID instanceID = 0; instanceID<_numInstances; ++instanceID) {
            ...
            boost::archive::binary_iarchive* iArchive = iArchiveWrapper.reset(BufReceive(instanceID, _query));
            size_t num = 0;
            (*iArchive) & num;
        ...
        for (InstanceID senderID = 0; senderID<_numInstances; ++senderID) {
            ...
            boost::archive::binary_iarchive* iArchive = iArchiveWrapper.reset(BufReceive(senderID, _query));
