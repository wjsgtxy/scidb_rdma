...
private:
    ...
    vector<AggIOMapping> collectIOMappings()
    {
        ...
        for (size_t i =0; i<_parameters.size(); i++)
        {
            if (_parameters[i]->getParamType() == PARAM_AGGREGATE_CALL)
            {
                ...
                else
                {
                    ...
                    {
                        if (inAttributeId.getId() == resultMappings[k].getInputAttributeId())
                        {
                    ...
                    {
                        resultMappings.push_back(AggIOMapping(inAttributeId.getId(), attID, agg));
                    }
                }
                attID++;
            }
        ...
        if (countStar)
        {
            ...
            if (resultMappings.size())
            {
                ...
                for (size_t i=0; i<resultMappings.size(); i++)
                {
                    size_t attributeSize =
                        _srcDesc.getAttributes().findattr(resultMappings[i].getInputAttributeId()).getSize();
                    if (attributeSize > 0)
                    {
            ...
            else
            {
                ...
                {
                    size_t attributeSize = _srcDesc.getAttributes().findattr(i).getSize();
                    if (attributeSize > 0 && !_srcDesc.getAttributes().findattr(i).isEmptyIndicator())
                    {
                        if (minSize == -1 || minSize > (int64_t) attributeSize)
                        {
                            minSize = attributeSize;
    ...
    static size_t estimateValueSize(AggIOMapping const& mapping, ArrayDesc const& dstDesc)
    {
        ...
        for (size_t i = 0; i < mapping.size(); ++i)
        {
            ...
            size_t varSize = dstDesc.getAttributes().findattr(mapping.getOutputAttributeId(i)).getVarSize();
    ...
    public:
            ...
            _eVSize(estimateValueSize(aggMapping, dstArray->getArrayDesc()))
        {
            ...
            const auto& dstAttrs = dstArray->getArrayDesc().getAttributes();
            for(size_t i=0; i<_nAggs; i++)
            {
                auto dstAttrIter = dstAttrs.find(aggMapping.getOutputAttributeId(i));
                SCIDB_ASSERT(dstAttrIter != dstAttrs.end());
                _daiters.push_back(dstArray->getIterator(*dstAttrIter));
...
public:
    ...
    void copyEmptyTagAttribute(std::shared_ptr<Array> & srcArray, std::shared_ptr<MemArray> & dstArray)
    {
        ...
        assert (srcArray->getArrayDesc().getAttributes().hasEmptyIndicator());
        std::shared_ptr<ConstArrayIterator> saiter = srcArray->getConstIterator(*srcArray->getArrayDesc().getEmptyBitmapAttribute());
        std::shared_ptr<ArrayIterator> daiter = dstArray->getIterator(*_schema.getEmptyBitmapAttribute());
        _localCellCount =0;
        while(!saiter->end())
        {
                                   ...
                                   size_t sizeLimit)
    {
        ...
        auto inputAttrIter = srcArrayAttrs.find(mapping.getInputAttributeId());
        SCIDB_ASSERT(inputAttrIter != srcArrayAttrs.end());
        std::shared_ptr<ConstArrayIterator> saiter = srcArray->getConstIterator(*inputAttrIter);
