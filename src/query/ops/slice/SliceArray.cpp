...
namespace scidb
{
    ...
    //
    SliceArrayIterator::SliceArrayIterator(SliceArray const& slice, const AttributeDesc& attrID)
    : ConstArrayIterator(slice),
      array(slice),
      inputIterator(slice.getPipe(0)->getConstIterator(attrID)),
      chunk(slice, attrID.getId()),
    ...
    //
    InfiniteSliceArrayIterator::InfiniteSliceArrayIterator(SliceArray const& slice,
    ...
    // Slice array methods
    //
    SliceArray::SliceArray(ArrayDesc& aDesc,
                           Coordinates const& aSlice,
                           uint64_t aMask,
    ...
    std::string const& SliceArray::getName() const
    {
        return desc.getName();
    }
    ...
    std::shared_ptr<ConstArrayIterator> SliceArray::getConstIteratorImpl(const AttributeDesc& attr) const
	{
		return std::shared_ptr<ConstArrayIterator>(useInfiniteIterator
                                              ? (ConstArrayIterator*)new InfiniteSliceArrayIterator(*this, attr)
                                              : (ConstArrayIterator*)new SliceArrayIterator(*this, attr));
	}
