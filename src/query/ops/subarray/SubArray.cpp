...
//
SubArrayIterator::SubArrayIterator(SubArray const& subarray,
                                   const AttributeDesc& attrID,
                                   bool doRestart)
: DelegateArrayIterator(subarray, attrID,
                        subarray.getPipe(0)->getConstIterator(attrID)),
  array(subarray),
...
MappedSubArrayIterator::MappedSubArrayIterator(SubArray const& subarray,
                                               const AttributeDesc& attrID):
...
{
    const AttributeDesc* inputAttribute = getPipe(0)->getArrayDesc().getEmptyBitmapAttribute();
    size_t nDims = getPipe(0)->getArrayDesc().getDimensions().size();
    std::shared_ptr<ConstArrayIterator> inputIter = getPipe(0)->getConstIterator(*inputAttribute);
...
DelegateArrayIterator* SubArray::createArrayIterator(const AttributeDesc& attrID) const
{
    if (_useChunkSet)
    {
