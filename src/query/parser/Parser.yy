...
%token<keyword>     DESC
%token<keyword>     DISTRIBUTION
%token<keyword>     DROP
%token<keyword>     EMPTYTAG
...
%type<node>         distribution
%type<node>         empty_tag_compression
...
nested_operand:
      '(' positional_operand ',' positional_operands ')' {$$ = _fac.newNode(nestedArgs,@$,$2,_fac.pop($4));}
    | '(' positional_operand ',' ')'                     {$$ = _fac.newNode(nestedArgs,@$,$2,cnodes());}
    ;
...
create_array_statement:
      CREATE temp ARRAY reference schema distribution empty_tag_compression
                                                         {$$ = _fac.newApp(@$,"Create_Array",$4,$5,$2,$6,$7);}
    | CREATE temp ARRAY reference schema distribution    {$$ = _fac.newApp(@$,"Create_Array",$4,$5,$2,$6);}
    | CREATE temp ARRAY reference schema                 {$$ = _fac.newApp(@$,"Create_Array",$4,$5,$2);}
    ;

schema:
      '<' attributes '>'  /* use dataframe dimensions */ {$$ = _fac.newNode(schema,@$,_fac.newList(@2,$2),0);}
    | '<' attributes '>' '[' dimensions ']'              {$$ = _fac.newNode(schema,@$,_fac.newList(@2,$2),
                                                                            _fac.newList(@5,$5),0);}
    ;

attribute:
     identifier ':' identifier nullable default compression reserve {$$ = _fac.newNode(attribute,@$,
                                                                                       $1,$3,$4,$5,$6,$7);}
    ;
...
empty_tag_compression:
      EMPTYTAG COMPRESSION constant_string               {$$ = $3;}
    ;

distribution:
      DISTRIBUTION identifier                            {$$ = _fac.newNode(distribution,@$,$2);}
    ;
...
update_array_statement:
    UPDATE reference SET updates where_clause            {$$ = _fac.newNode(updateArray,@$,
                                                                            $2,_fac.newList(@4,$4),$5);}
    ;
...
redimension_clause:
      REDIMENSION BY '[' dimensions ']'                  {$$ = _fac.newNode(redimensionClause,@$,
                                                                            _fac.newList(@4,$4));}
    ;
...
void Parser::error(const location_type& w, const std::string& s)
{
