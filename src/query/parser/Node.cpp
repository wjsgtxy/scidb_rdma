 ...
 *
 * @note This brashly assumes that the reconstituted query string
 *       produced by the Unparser is useful for debugging.  If you
 *       disagree, try AstWriter instead.
 */
...
#   undef X
    default:
        ::snprintf(buf, 128, "type_%d", int(t));
        return &buf[0];
    }
}
