...
void mpiErrorHandler(MPI::Comm& comm, int *a1, ...)
{
    ...
    int err_len;
    MPI::Get_error_string(*a1, err_str, err_len);
    cerr << "Abort Error: " << err_str << std::endl;
    ::abort();
}
...
int main(int argc, char* argv[])
{
    ...
    if (argc>MIN_ARGC) {
      ...
      std::cerr << "START DELAY="<< argv[START_DELAY_INDEX] << std::endl;
    ...
    catch (const scidb::SystemException &e)
    {
        ...
        // TODO: further review:  can we get a stack trace and still MPI_Abort() to clean up faster/better? (j.mq)
        e.raise();
    }
