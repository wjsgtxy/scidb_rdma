...
ErrorsLibrary::ErrorsLibrary()
{
    ...
    registerErrors(CORE_ERROR_NAMESPACE, &_builtinLongErrorsMsg);
}

void ErrorsLibrary::registerErrors(const std::string &errorsNamespace, ErrorsMessages* msgMap)
{
    ScopedMutexLock lock(_lock, PTW_SML_ERRORS_LIBRARY);
    int badErrorNumber = 0;

    if (_errorNamespaces.find(errorsNamespace) != _errorNamespaces.end()) {
        throw SYSTEM_EXCEPTION(SCIDB_SE_ERRORS_MGR, SCIDB_LE_ERRNS_ALREADY_REGISTERED)
            << errorsNamespace;
    }

    if (CORE_ERROR_NAMESPACE != errorsNamespace)
    {
...
void ErrorsLibrary::unregisterErrors(const std::string &errorsNamespace)
{
    ...
    if (CORE_ERROR_NAMESPACE == errorsNamespace) {
        throw SYSTEM_EXCEPTION(SCIDB_SE_ERRORS_MGR, SCIDB_LE_ERRNS_CAN_NOT_BE_UNREGISTERED)
            << errorsNamespace;
    }
...
{
    // These are fixed at startup, so no locking is needed.
...
{
    if (errorsNamespace == CORE_ERROR_NAMESPACE) {
        // Fixed at startup, no locking needed.
        auto p = _builtinLongErrorsMsg.find(longError);
        if (p == _builtinLongErrorsMsg.end()) {
            return boost::str(boost::format(
                "!!!Cannot obtain error message for error code '%1%'"
