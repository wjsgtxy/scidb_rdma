...
void DeepChunkMerger::merge()
{
    ...
    // Get payloads.  DeepChunkMerger is within the Chunk trust boundary.
    _payloadDst.reset(new ConstRLEPayload(
        reinterpret_cast<const char*>(_dst.getWriteDataImpl())));
    _payloadWith.reset(new ConstRLEPayload(
        reinterpret_cast<const char*>(_with.getConstDataImpl())));
...
void DeepChunkMerger::mergeEmptyBitmapChunks()
{
    ...
    for (ConstRLEEmptyBitmap::Segment const& segment :  _intermediateBitmapSegments) {
        mergedBitmap.addSegment(segment);
    }
    _dst.allocate(mergedBitmap.packedSize());
    // inside trust boundary
    mergedBitmap.pack(reinterpret_cast<char*>(_dst.getWriteDataImpl()));
    _dst.write(_query);
}
...
void DeepChunkMerger::mergeAtSegmentLevel()
{
    ...
    _bitmapDst.reset(new ConstRLEEmptyBitmap(
        reinterpret_cast<const char*>(_dst.getWriteDataImpl()) + _payloadDst->packedSize()));
    _bitmapWith.reset(new ConstRLEEmptyBitmap(
        reinterpret_cast<const char*>(_with.getConstDataImpl()) + _payloadWith->packedSize()));
    ...
    // inside trust boundary
    mergedPayload.pack((char*)_dst.getWriteDataImpl());
    mergedBitmap.pack((char*)_dst.getWriteDataImpl() + mergedPayload.packedSize());
    _dst.write(_query);
}
...
void DeepChunkMerger::fillMergedPayloadUsingIntermediateResult(RLEPayload& mergedPayload)
{
    ...
    for (IntermediatePayloadSegment const& inputSegment : _intermediatePayloadSegments) {
