...
bool BufferedFileInput::initialize()
{
    ...
    if (!theBuffer._loaded) {
        try {
            ...
            Event::ErrorChecker ec = std::bind(&queryAndScannerValid,
...
void BufferedFileInput::FillBufferJob::run()
{
    ...
    Event::ErrorChecker ec = std::bind(&queryAndScannerValid, _query, _pState);
...
BufferedFileInput::~BufferedFileInput()
{
    ...
    if (_fillBufferJob) {
        ...
        _fillBufferJob->wait(PTW_JOB_FILEREAD);
    }
}
