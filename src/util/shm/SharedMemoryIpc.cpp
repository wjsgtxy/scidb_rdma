...
void SharedMemory::create(AccessMode amode)
{
    ...
    try {
        _shm.reset(new bipc::shared_memory_object(bipc::create_only, getName().c_str(),
                                            static_cast<bipc::mode_t>(amode)));
    } catch(bipc::interprocess_exception &ex) {
        std::cerr << "shm::create error:" << ex.what() << std::endl;
        if (ex.get_error_code() == bipc::already_exists_error) {
            throw AlreadyExistsException(ex.get_native_error(), REL_FILE, __FUNCTION__, __LINE__);
        }
        if (ex.get_error_code() !=  bipc::no_error) {
...
void SharedMemory::open(AccessMode amode)
{
    ...
    try {
        _shm.reset(new bipc::shared_memory_object(bipc::open_only, getName().c_str(),
                                            static_cast<bipc::mode_t>(amode)));
    } catch(bipc::interprocess_exception &ex) {
        std::cerr << "shm::open error:" << ex.what() << std::endl;
        if (ex.get_error_code() == bipc::not_found_error) {
...
void* SharedMemory::get()
{
    if (!_region) {
        ...
        try {
            ...
            _region.reset(new bipc::mapped_region((*_shm), _shm->get_mode()));
        } catch(bipc::interprocess_exception &ex) {
            if (ex.get_error_code() !=  bipc::no_error) {
...
void SharedFile::create(AccessMode amode)
{
    ...
    createFile();
    try {
        _file.reset(new bipc::file_mapping(getName().c_str(),
                                     static_cast<bipc::mode_t>(amode)));
    } catch(bipc::interprocess_exception &ex) {
...
uint64_t SharedFile::getSize() const
{
    ...
    bipc::file_handle_t fh = bipc::ipcdetail::file_handle_from_mapping_handle(_file->get_mapping_handle());
    bool rc = bipc::ipcdetail::get_file_size(fh, n);
#else
     bipc::file_handle_t fh = detail::file_handle_from_mapping_handle(_file->get_mapping_handle());
...
void* SharedFile::get()
{
    if (!_region) {
        ...
        try {
            // map the whole shared memory in this process
            _region.reset(new bipc::mapped_region((*_file), _file->get_mode()));
