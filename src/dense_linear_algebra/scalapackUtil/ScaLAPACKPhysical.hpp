...
#include <system/BlockCyclic.h>   // for procRowCol_t
...
void checkBlacsInfo(std::shared_ptr<Query>& query, const slpp::context_t&,
...
public:
     ...
     * between ScaLAPACK-based operators (which have distribution dtScaLAPACK) and others
     * (e.g. store) which require dtHashPartitioned.  However, this requires using store() to
     ...
     * @return              outputArray redistributed to dtHashPartitioned.
     */
     ...
     */
    slpp::context_t                 doBlacsInit(std::vector< std::shared_ptr<Array> >& redistributedInputs, std::shared_ptr<Query>& query, const std::string& callerLabel);
