                                              ...
                                              ArrayDesc& outSchema)
{
    ...
    //
    slpp::context_t blacsContext = doBlacsInit(inputArrays, query, "MPICopyPhysical");
...
{
    //
    // + converts inputArray[0] to dtScaLAPACK distribution
    // + calls invokeMPI()
    ...
    // to
    // ScaLAPACK tiles (efficient cache-aware size) and dtScaLAPACK, which is
    ...
    InstanceID instanceID = slpp::int_cast(query->getInstanceID());
    ...
    procRowCol_t MN = { safe_static_cast<procNum_t>(nRows), safe_static_cast<procNum_t>(nCols)};
    procRowCol_t MNB = { safe_static_cast<procNum_t>(dims[0].getChunkInterval()),
                         safe_static_cast<procNum_t>(dims[1].getChunkInterval()) };
