...
public:
    ...
    void invokeMPIRank(std::vector< std::shared_ptr<Array> >* inputArrays,
                       ...
                       const slpp::context_t& blacsContext,
                       ArrayDesc& outSchema,
...
std::shared_ptr<Array> MPIRankPhysical::execute(std::vector< std::shared_ptr<Array> >& inputArrays, std::shared_ptr<Query> query)
{
    ...
    slpp::int_t IC = slpp::int_cast(instanceID);
                                     ...
                                     slpp::int_t* INFO)
{
    ...
    if (membership->getNumInstances() != query->getInstancesCount()) {
        ...
        throw USER_EXCEPTION(SCIDB_SE_EXECUTION, SCIDB_LE_NO_QUORUM)
            << "invokeMPIRank: instance count mismatch";
    }
    ...
    for(int ii=0; ii < SIZE_IN; ii++) {
        IN[ii] = static_cast<double>(MYPE) ; // what we think the rank will be in the MPI process
    }
