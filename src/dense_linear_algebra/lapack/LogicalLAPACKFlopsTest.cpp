...
/**
 * @brief The operator: _lapackflopstest().
 *
 * @par Synopsis:
 *   _lapackflopstest()
 *
 ...
 *   It runs a dgesvd_() locally on each instance to
...
public:
    ...
    ArrayDesc inferSchema(std::vector<ArrayDesc> schemas, std::shared_ptr<Query> query)
    {
        ...
        // only timing one operation, dgesvd, at the moment
        ...
        return ArrayDesc("lapack_flops_test", attributes, dimensions,
                         createDistribution(dtRowCyclic), query->getDefaultArrayResidency());
    }
};

REGISTER_LOGICAL_OPERATOR_FACTORY(LogicalBLASFlopsTest, "_lapackflopstest");
