                     ...
                     ExtractOp_tt& extractOp, const std::shared_ptr<Query>& query)
{
    ...
    const auto& attrs = arrayDesc.getAttributes();
    scidb::AttributeDesc const& attrDesc = attrs.findattr(attrID);
    ...
    for(chunksIt = array->getConstIterator(attrs.firstDataAttribute()); !chunksIt->end(); ++(*chunksIt) ) {
