...
slpp::int_t pdgesvdSlave2(const slpp::int_t ICTXT, PdgesvdArgs args, void* bufs[], size_t sizes[], unsigned count)
{
    ...
    if (MYPNUM==0 && DBG) {
        ...
        ssize_t numToPrintAtEnd=4;
        for(ssize_t jj=std::max(ii, SIZE_S-numToPrintAtEnd); jj < SIZE_S; jj++) {
            std::cerr << "pdgesvdSlave: S["<<jj<<"] = " << S[jj] << std::endl;
        }
    }
    if (DBG) {
        if (args.jobU == 'V') {
            for(ssize_t ii=0; ii < std::min(SIZE_U, numToPrintAtStart); ii++) {
                std::cerr << "pdgesvdSlave: U["<<ii<<"] = " << U[ii] << std::endl;
            }
        }
        if (args.jobVT == 'V') {
            for(ssize_t ii=0; ii < std::min(SIZE_VT, numToPrintAtStart); ii++) {
