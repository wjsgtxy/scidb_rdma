...
TileApplyChunkIterator::TileApplyChunkIterator(TileApplyArrayIterator const& arrayIterator,
    ...
    , _exp ( (*_array._expressions)[arrayIterator.attr.getId()].get() )
    , _needCoordinates(false)
    , _bindings(_array._bindingSets[arrayIterator.attr.getId()]
                ? (*_array._bindingSets[arrayIterator.attr.getId()])
                : _fakeBinding)
    , _iterators(_bindings.size())
    , _params(*(*_array._expressions)[arrayIterator.attr.getId()])
    , _mode(iterationMode)
    , _applied(false)
    , _nullable(_array._attributeNullable[arrayIterator.attr.getId()])
...
: DelegateArrayIterator(array, outAttrID, array.getPipe(0)->getConstIterator(inAttrID)),
  _iterators(array._bindingSets[outAttrID.getId()] ? array._bindingSets[outAttrID.getId()]->size() : 0),
  _inputAttrID(inAttrID.getId())
{
    auto outAttrIDVal = outAttrID.getId();
    auto inAttrIDVal = inAttrID.getId();
    vector<BindInfo> const fakeBinding(0);
    vector<BindInfo> const& bindings = array._bindingSets[outAttrIDVal] ? *array._bindingSets[outAttrIDVal] : fakeBinding;
    std::shared_ptr<Array> inputArray = array.getPipe(0);
    ...
    for (size_t i = 0, n = _iterators.size(); i < n; ++i)
    {
        ...
        {
        case BindInfo::BI_ATTRIBUTE:
        if (resolvedId == inAttrIDVal)
        {
            _iterators[i] = inputIterator;
        }
        else
        {
            const auto& attr = inputArray->getArrayDesc().getAttributes().findattr(resolvedId);
            _iterators[i] = inputArray->getConstIterator(attr);
        }
        break;
        case BindInfo::BI_COORDINATE:
        break;
...
DelegateArrayIterator* TileApplyArray::createArrayIterator(const AttributeDesc& attrID_in) const
{
    ...
    {
        inputAttrID = getPipe(0)->getArrayDesc().getEmptyBitmapAttribute()->getId();
    }
    else
    {
        inputAttrID = attrID;
    ...
    auto _inputAttrID = getPipe(0)->getArrayDesc().getAttributes().findattr(inputAttrID);
    return new TileApplyArrayIterator(*this, attrID_in, _inputAttrID);
}
