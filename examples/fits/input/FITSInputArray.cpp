...
void FITSInputArray::initValueHolders()
{
    for (const auto& attr : desc.getAttributes()) {
        values[attr.getId()] = Value(TypeLibrary::getType(attr.getType()));
    }
}
...
std::shared_ptr<ConstArrayIterator> FITSInputArray::getConstIteratorImpl(const AttributeDesc& attr) const
{
    return std::shared_ptr<ConstArrayIterator>(new FITSInputArrayIterator(*(FITSInputArray*) this, attr.getId()));
}
...
bool FITSInputArray::validSchema()
{
    ...
    const TypeId attrType = attrs.firstDataAttribute().getType();
...
void FITSInputArray::initMemChunks(std::shared_ptr<Query>& query)
{
    ...
    for (const auto& attr : desc.getAttributes()) {
        ...
        MemChunk& chunk = chunks[attr.getId()].chunks[chunkIndex % kWindowSize];
        chunk.initialize(this, &desc,
                         addr, attr.getDefaultCompressionMethod());
